...
public class LoginPageHelper extends DriverHelper {
	...
	public void Login(String username, String password) {
		try {
			String usernametextbox = loginpageLocator
					.getLocator("Login.UserName");
			String passwordtextbox = loginpageLocator
					.getLocator("Login.Password");
			String loginbutton = loginpageLocator
					.getLocator("Login.LoginButton");
			String verifylogin = loginpageLocator
					.getLocator("Login.VerifyLogin");
			String verifycollection = loginpageLocator
					.getLocator("Login.VerifyCollection");
			WaitForElementPresent(loginbutton, 30);
			Boolean Usernamebutton = isElementPresent(usernametextbox);
			System.out.println("User Name button element is present: "
					+ Usernamebutton);
			sendKeys(usernametextbox, username);
			Reporter.log("Enter username '" + username
					+ "' into username text box '" + usernametextbox + "'",
					true);
			Boolean Passwordbutton = isElementPresent(passwordtextbox);
			System.out.println("Password button element is present: "
					+ Passwordbutton);
			sendKeys(passwordtextbox, password);
			Reporter.log("Enter password '" + password
					+ "' into password text box '" + passwordtextbox + "'",
					true);
			Boolean Loginbutton = isElementPresent(loginbutton);
			System.out.println("Login button element is present: "
					+ Loginbutton);
			clickOn(loginbutton);
			...
			System.out.println("Login text " + verifyLogin + " is available "
					+ textavailable);
			WaitForElementNotPresent(verifycollection, 10);
			...
			System.out.println("Login text " + verifyCollection
					+ " is available " + textavailable1);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
