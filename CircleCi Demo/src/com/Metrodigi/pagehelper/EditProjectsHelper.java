...
public class EditProjectsHelper extends DriverHelper {
	...
	/**
	 * @description This method is used to Edit Collection Details to the
	 *              application
	 * @return none
	 */
	public String NewCollection() {
		...
		try {
			String CollectionName = editprojectsLocator
					.getLocator("EditProjects.Collection.CollectionName");
			collectionname = propertyReader
					.readApplicationFile("Collectionname1")
					+ String.valueOf(getRandomInteger(1, 999));
			String AddProjectButton = editprojectsLocator
					.getLocator("EditProjects.Collection.AddProjectButton");
			...
			String savebutton = editprojectsLocator
					.getLocator("EditProjects.NewProject.Savebutton");
			clickOn(savebutton);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String CollectionDetails() {
		...
		try {
			...
			Reporter.log("Click on Add Project button:" + AddProjectButton,
					true);
			WaitForElementNotPresent(AddProjectButton, 10);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String AddProject() {
		...
		try {
			String AddProjectButton = editprojectsLocator
					.getLocator("EditProjects.Collection.AddProjectButton");
			clickOn(AddProjectButton);
			Reporter.log("Click on Add Project button of existing Collection:"
	...
	/**
	 * @description This method is used to Add New Project with Uploaded file to
	 *              the application
	 * @return none
	 */
	public String Newproject() {
		...
		try {
			...
			String reflowable = editprojectsLocator
					.getLocator("EditProjects.NewProject.Reflowable");
			Boolean reflowableelementpresent = isElementPresent(reflowable);
			System.out.println("Reflowable Element is present: "
					+ reflowableelementpresent);
			clickOn(reflowable);
			Reporter.log("Click on Reflowable radio button:" + reflowable, true);

			/*
			 * Click on Fixed layout radio button String fixedlayout =
			 * editprojectsLocator
			 * .getLocator("EditProjects.NewProject.Fixedlayout"); Boolean
			 * fixedlayoutelementpresent = isElementPresent(fixedlayout);
			 * System.out.println("Fixed layout Element is present: "+
			 * fixedlayoutelementpresent); clickOn(fixedlayout);
			 * Reporter.log("Click on fixedlayout radio button:"+fixedlayout,
			 * true);
			 */
			...
			// Click on Do you have files to convert? Yes or No radio button
			  ...
			  clickOn(yes); Reporter.log("Click on Yes radio button:"+yes,
			  true);*/
			 
			String no = editprojectsLocator
					.getLocator("EditProjects.NewProject.No");
			...
			String Projectname = editprojectsLocator
					.getLocator("EditProjects.NewProject.ProjectName");
			projectName = propertyReader.readApplicationFile("Projectname1")
					+ String.valueOf(getRandomInteger(1, 999));
			Boolean Projectnameelementpresent = isElementPresent(Projectname);
			System.out.println("Project Name Element is present: "
					+ Projectnameelementpresent);
    ...
    // Upload Files
     ...
     clickOn(locatefiles);
     uploadDocxfile();
     WaitForElementNotPresent(savebutton, 100);
	 ...
	 /**
     * @description This method is used to click on save button
     * @return none
     */
	...
	public void uploadepub() {
		try {
			String locatefiles = editprojectsLocator
					.getLocator("EditProjects.NewProject.LocateFilesButton");
			String savebutton = editprojectsLocator
					.getLocator("EditProjects.NewProject.Savebutton");
			// Upload Files
			clickOn(locatefiles);
